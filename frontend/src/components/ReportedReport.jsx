...
const updatePhotoStatus = async (id, status) => {
  try {
    const response = await axios.post(`http://localhost:3000/photo/in-progress`, {
...
const PotholeDescriptionCard = ({ 
  ...
  const markAsCompleted = async () => {
    setReportStatus('progress'); // Update the local state to 'completed'
    await updatePhotoStatus(id, 'progress'); // Send the request to the backend
  };
  ...
  />
        ...
        <div className="w-2/3 p-5">
          ...
          {reportStatus !== 'completed' && (
            <div className="mb-4">
              <button 
                onClick={() => updatePhotoStatus(id, 'in-progress')}
          ...
          {reportStatus === 'in-progress' && (
            <div className="mb-4">
              <p className="text-sm text-yellow-600">This pothole is being attended to.</p>
              <button 
